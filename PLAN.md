...
- **Phase 1** *(in progress — separate session per rule)* — the four starter rules, each plugged into the scaffolding:
  ...
  - Rule 2 ✅ Section symbol placement (`LS-CITE-02`, Tier 3) — shipped. Bidirectional check: `§` / `§§` at paren-depth 0 fires toward "section"/"sections"; "section"/"sections" at paren-depth > 0 (when followed by a statute-number-shaped token) fires toward `§`/`§§`. Paragraph-local paren-depth scanner treats both `(` and `[` as openers (LS uses brackets for sub-references inside citation sentences, so `[… § 9.1 …]` must not fire). Scanner lives in the rule dir; promotes to `shared/` when LS-CITE-03 lands and needs the same split. CA state-court behavior only; federal `§`-in-prose deviation documented in `rules/02-section-symbol/NOTES.md` pending `court_context` plumbing (open question #6).
