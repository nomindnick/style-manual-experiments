# LS-CAP-02 — prose-prompts arm research log

Lab notebook for the prose-prompts arm of the district/board/etc.
capitalization rule research. Each experiment records the corpus, the
prompt, the models, and the results in one place so we can see what's
been tried and where the dial moved. Experiments here iterate
natural-language prompt content, target-marker formats, and sampling
parameters.
